...
export default function CustomLink({ link, text, style, target }) {
  return (
      <Link className="relative" href={`${link ? link : "/"}`} target={target}>
        ...
        >
          <span className={`relative  transition after:duration-700 inline-flex items-center sm:text-2xl text-2.5xl font-normal leading-7 -tracking-sm after:content-['↗'] after:relative after:-bottom-1 after:text-2.5xl after:lg:text-2xl after:lg:group-hover:-top-1 after:lg:group-hover:-right-1 ${style} `}>{text}</span>
          <span
